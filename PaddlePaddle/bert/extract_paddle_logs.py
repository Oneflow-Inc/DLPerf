...
def extract_info_from_file(log_file, result_dict, speed_dict):
    ...
    total_batch_size = card_num * batch_size
    tmp_dict = {
        'average_speed': 0,
    ...
    with open(log_file) as f:
        lines = f.readlines()
        for line in lines: # , speed: 0.163902 steps/s
            if "speed:" in line:
                p1 = re.compile(r" speed: ([0-9]+\.[0-9]+) steps/s", re.S)
                item = re.findall(p1, line)
                a = float(item[0].strip())
                speed = float(item[0].strip())*total_batch_size
                avg_speed_list.append(round(speed, 4))
