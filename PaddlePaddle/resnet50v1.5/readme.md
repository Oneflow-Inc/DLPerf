...
NODE2节点`models/PaddleCV/image_classification/`目录下，修改run_two_node.sh脚本中的`CURRENT_NODE=$NODE2`，再执行`bash run_two_node.sh`，即可运行2机16卡的训练，同样默认测试6组。
...
```shell
python extract_paddle_logs.py  --log_dir=logs/paddle/resnet50 --batch_size_per_device=128
```
