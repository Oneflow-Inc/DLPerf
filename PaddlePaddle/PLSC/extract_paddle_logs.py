...
parser.add_argument("--log_dir", type=str, default="./logs/paddle-plsc/arcface", required=True)
...
def extract_info_from_file(log_file, result_dict, speed_dict):
    ...
    avg_speed_list = []
    start_str = "Pass:" + str(args.warmup_batches) + " batch:"
    temp_num = args.warmup_batches
    end_ste  = "Pass:" + str(args.train_batches) + " batch:"
    # extract info from file content
    with open(log_file) as f:
        lines = f.readlines()
        for line in lines:
            if "Pass:0 batch:" in line:
                p1 = re.compile(r" qps:([0-9]+\.[0-9]+)", re.S)
                item = re.findall(p1, line)
                a = float(item[0].strip())
                avg_speed_list.append(a)

    # compute avg throughoutput
    avg_speed = round(np.mean(avg_speed_list[args.warmup_batches//10*node_num:args.train_batches//10*node_num]), 2)
