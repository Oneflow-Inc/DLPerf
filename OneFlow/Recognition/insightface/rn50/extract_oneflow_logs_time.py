...
parser.add_argument("-wb", "--warmup_batches", type=int, default=50)
parser.add_argument("-tb", "--train_batches", type=int, default=150)
...
def extract_info_from_file(log_file, result_dict, speed_dict):
    ...
    pt = re.compile(r"throughput: (.*)", re.S)
    start_time = ''
    end_time = ''
    line_num = 0
    thoughput_data = []
    with open(log_file) as f:
        ...
        for line in lines:
            if "train: iter" in line:
                pt1 = re.compile(r"train: iter (.*), loss")
                skip_time = int(re.findall(pt1, line)[0])
                line_num += 1
                    
                if line_num >= args.warmup_batches:
                    thoughoutput = float(re.findall(pt, line)[0])
                    thoughput_data.append(thoughoutput)

                    if line_num == args.train_batches-1:
                        iter_num = args.train_batches - args.warmup_batches
                        avg_speed = round(np.mean(thoughput_data), 2)
                        break
