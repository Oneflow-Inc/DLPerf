...
LOCAL_RUN=${TEST_SCRIPTS}/train_insightface.sh
...
for host in "${hosts[@]:1}"; do
  ...
  scp -P ${port} -r $SCRIPTS_PATH $TEST_SCRIPTS/* $host:~/oneflow_temp
done

function train_once() {
  for host in "${hosts[@]:1}"; do
       ssh -p ${port} $host "cd ~/oneflow_temp; nohup bash ~/oneflow_temp/train_insightface.sh ~/oneflow_temp/oneflow_face ${network} ${dataset} ${loss} ${num_nodes} $bz_per_device $train_unit $train_iter ${gpu_num_per_node} $precision $model_parallel $partial_fc $1 $sample_ratio $num_classes 1>${log_file} "
  done
  host=${hosts[0]}
  echo "start training on ${host}"
  ssh -p ${port} $host "rm -rf ~/oneflow_temp/*"
  scp -P ${port} -r $SCRIPTS_PATH $TEST_SCRIPTS/* $host:~/oneflow_temp
  ssh -p ${port} $host "cd ~/oneflow_temp; bash ~/oneflow_temp/train_insightface.sh ~/oneflow_temp/oneflow_face ${network} ${dataset} ${loss} ${num_nodes} $bz_per_device $train_unit $train_iter ${gpu_num_per_node} $precision $model_parallel $partial_fc $1 $sample_ratio $num_classes 1>${log_file}"
