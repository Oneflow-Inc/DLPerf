...
3. 在`vocab_size`、`batch_size`等参数相同的条件下，单机8GPU时OneFlow 的GPU显存占用最低为hugeCTR的19%；4机32GPU时GPU显存占用最低仅为hugeCTR的11%，意味着 OneFlow 对显存的利用和管理能力更强，并且随着节点数和gpu数增加，OneFlow领先的趋势更佳明显。